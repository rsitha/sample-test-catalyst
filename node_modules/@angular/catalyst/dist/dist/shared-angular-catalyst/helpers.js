...
import { ChangeDetectorRef, Component, destroyPlatform, NgZone, RendererFactory2 } from '@angular/core';
...
const syncTestZoneSpecCtor = Zone['SyncTestZoneSpec'];
// tslint:disable-next-line:no-any Zone shenanigans.
const fakeAsyncTestZoneSpecCtor = Zone['FakeAsyncTestZoneSpec'];
...
const globalInternal = (typeof window === 'undefined' ? global : window);
...
function initTestEnvironment(config) {
    var _a, _b, _c;
    try {
        TestBed.initTestEnvironment(BrowserDynamicTestingModule, platformBrowserDynamicTesting(), {
            teardown: {
                destroyAfterEach: !!((_a = config.teardown) === null || _a === void 0 ? void 0 : _a.destroyAfterEach),
                // `rethrowErrors` should default to true, per interface
                // specification.
                rethrowErrors: (_c = (_b = config.teardown) === null || _b === void 0 ? void 0 : _b.rethrowErrors) !== null && _c !== void 0 ? _c : true,
            }
        });
    }
    catch (e) {
 ...
 */
export function xdescribe(description, specDefinitions) {
    console.warn(`Catalyst: 'xdescribe()' is deprecated. ` +
 ...
 */
export function fdescribe(description, specDefinitions) {
    console.warn(`Catalyst: 'fdescribe()' is deprecated. ` +
...
function itLegacyInternal(description, specDefinitions, timeout) {
    const originalIt = getOriginalFunction('it');
    return originalIt(description, wrapTestInFakeAsyncZone(specDefinitions, false /* flush */), timeout);
}
/**
...
function itAsync(description, specDefinitions, timeout) {
    const originalIt = getOriginalFunction('it');
    return originalIt(description, () => __awaiter(this, void 0, void 0, function* () {
        inCatalyst = true;
...
function xitAsync(description, specDefinitions, timeout) {
    ...
    return originalXit(description, () => __awaiter(this, void 0, void 0, function* () {
        inCatalyst = true;
        yield specDefinitions();
        inCatalyst = false;
    }), timeout);
...
function fitAsync(description, specDefinitions, timeout) {
    ...
    return originalFit(description, () => __awaiter(this, void 0, void 0, function* () {
        inCatalyst = true;
        yield specDefinitions();
        inCatalyst = false;
    }), timeout);
 ...
 * @param timeout optional timeout in milliseconds
 */
export function xit(description, specDefinitions, timeout) {
 ...
 * @param timeout optional timeout in milliseconds
 */
export function fit(description, specDefinitions, timeout) {
 ...
 */
export function fitLegacy(description, specDefinitions, timeout) {
    console.warn(`Catalyst: 'fit()' is deprecated. Use 'it.only()' instead`);
...
function fitLegacyInternal(description, specDefinitions, timeout) {
    const originalFit = getOriginalFunction('fit');
    return originalFit(description, wrapTestInFakeAsyncZone(specDefinitions, false /* flush */), timeout);
}
...
/** Catalyst wrapper to describe. */
export const describe = describeInternal;
...
export const describeWithDate = describeWithDateInternal;
...
/** Catalyst wrapper to it. */
export const it = itInternalStrict;
/**
 ...
 */
export function beforeEachLegacy(specDefinitions, timeout) {
    const originalBeforeEach = getOriginalFunction('beforeEach');
    return originalBeforeEach(wrapTestInFakeAsyncZone(specDefinitions, false /* flush */), timeout);
}
/**
...
function beforeEachAsync(specDefinitions, timeout) {
    const originalBeforeEach = getOriginalFunction('beforeEach');
    return originalBeforeEach(() => __awaiter(this, void 0, void 0, function* () {
        inCatalyst = true;
        yield specDefinitions();
        inCatalyst = false;
    }), timeout);
 ...
 * @param timeout optional timeout in milliseconds
 */
export function beforeAll(specDefinitions, timeout) {
 ...
 * @param timeout optional timeout in milliseconds
 */
export function afterEach(specDefinitions, timeout) {
 ...
 */
export function afterEachLegacy(specDefinitions, timeout) {
    const originalAfterEach = getOriginalFunction('afterEach');
    return originalAfterEach(wrapTestInFakeAsyncZone(specDefinitions, false /* flush */), timeout);
}
/**
 * Catalyst wrapper to 'afterAll'.
 * @param timeout optional timeout in milliseconds
 */
export function afterAll(specDefinitions, timeout) {
...
function wrapTestInFakeAsyncZone(specDefinitions, flush = true, strict = false) {
    return function (...args) {
        ...
        getFakeAsyncZone().run(() => {
            ...
            if (unsafeAsync) {
                if (strict) {
                    fail(`Test returned a promise that didn't resolve within the fake ` +
                        `async zone. It either never completes or waits for real async ` +
                        `data (use it.async in this case).`);
                }
                else {
                    console.warn(`Test returned a promise that didn't resolve within the fake ` +
                        `async zone. It either never completes or waits for real async ` +
 ...
 */
function getScopeProvider(config) {
    return {
        provide: '$scope',
...
function setupModuleInternal(config) {
...
function createComponentInternal(component, inputs = {}, beforeChangeDetection, flush = true) {
    compileComponents();
...
function bootstrapInternal(component, inputs = {}, beforeChangeDetection, flush = true) {
    if (!isTestModuleSetup) {
        throw new Error('Please call setupModule() before bootstrap()');
    }
    const createdComponent = createComponent(component, inputs, beforeChangeDetection, flush);
...
let DynamicTemplateComponent = class DynamicTemplateComponent {
    constructor() {
...
function bootstrapTemplateInternal(templateString, instance, beforeChangeDetection, flush = true) {
    if (!isTestModuleSetup) {
    ...
    function getComponent(template, instance) {
        componentInstance = instance;
        TestBed.overrideTemplate(DynamicTemplateComponent, template);
        return DynamicTemplateComponent;
    }
    const createdComponent = createComponent(getComponent(templateString, instance), {}, beforeChangeDetection, flush);
    if (BAD_ANIMATION_MODULES.includes(TestBed.inject(ANIMATION_MODULE_TYPE, ''))) {
        console.error(new Error(BAD_ANIMATION_MODULE_MSG));
    }
    return createdComponent;
...
function nowInternal(fn) {
    ...
    if (fixture != null) {
        ...
        getFakeAsyncZone().run(() => __awaiter(this, void 0, void 0, function* () {
            retVal = yield fn();
        }));
    }
    flushMicroTasks();
 ...
 *   }
 */
export function setupNg1Module(ng1Module, ng2Module, cachedTemplateModuleName, 
 ...
 */
export function compileNg1Component(ng1Componentselector, $scope) {
 ...
 * @return AngularJS element created
 */
export function createNg1Component(ng1Module, ng1Componentselector, ng2Module, cachedTemplateModuleName) {
...
function callInFakeAsyncZone(fn) {
    if (isInFakeAsyncZone()) {
        return fn();
    }
    else {
        return getFakeAsyncZone().run(fn);
    }
}
function createComponent(component, inputs = {}, beforeChangeDetection, flush = true) {
    return callInFakeAsyncZone(() => createComponentInternal(component, inputs, beforeChangeDetection, flush));
}
...
function assertIsInCatalyst(msg = '') {
    if (!inCatalyst) {
        throw new Error(msg + ` Remember to import 'it', 'beforeEach', 'afterEach' etc. ` +
 ...
 */
export function setupModule(config) {
 ...
 */
export function bootstrap(component, inputs = {}, beforeChangeDetection, flush = true) {
    assertIsInCatalyst('Put bootstrap() inside beforeEach, beforeAll or it.');
    return callInFakeAsyncZone(() => bootstrapInternal(component, inputs, beforeChangeDetection, flush));
}
/**
 ...
 * @return root component instance
 */
export function bootstrapTemplate(templateString, instance = {}, beforeChangeDetection, flush = true) {
    assertIsInCatalyst('Put bootstrapTemplate() inside beforeEach, beforeAll or it.');
    return callInFakeAsyncZone(() => bootstrapTemplateInternal(templateString, instance, beforeChangeDetection, flush));
}
/**
 ...
 */
export function getDebugEl(query, root) {
    assertIsInCatalyst('Put getDebugEl() inside beforeEach, beforeAll, it, afterAll ' +
        'or afterEach.');
 ...
 */
export function getDebugEls(query, root) {
    assertIsInCatalyst('Put getDebugEls() inside beforeEach, beforeAll, it, afterAll ' +
        'or afterEach.');
 ...
 */
export function hasDebugEl(query, root) {
    assertIsInCatalyst('Put hasDebugEl() inside beforeEach, beforeAll, it, afterAll ' +
        'or afterEach.');
 ...
 */
export function trigger(el, eventType, eventParam, extraParams) {
